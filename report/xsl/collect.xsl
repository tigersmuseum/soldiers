...
<!-- 
	...
	Soldiers XML generated by querying a soldiers database. This will have one or more person elements, each having a @sid attribute.
...
<xsl:template match="soldiers:person[@sid]">
<bio sid="{@sid}">
	<database>
		<xsl:copy-of select="."/>
	</database>
...
<xsl:template match="soldiers:person">
	<xsl:message>Error - No @sid attribute for <xsl:value-of select="soldiers:surname"/>, <xsl:value-of select="soldiers:initials"/> </xsl:message>
</xsl:template>
